...
public class JSONMapperPunkt {
    ...
    static public String getJSON(List<PunktPomiarowy> data){
        ...
        for(PunktPomiarowy punkt : data){
            json.append(getRow(punkt)).append(",");;
        }
    ...
    static private StringBuilder getRow(PunktPomiarowy punkt){
        ...
        row.append("city: ").append(!punkt.getCity().equals("") ? punkt.getCity() : "null").append(",");
        row.append("street: ").append(!punkt.getStreet().equals("") ? punkt.getStreet().replace(',','+').replace('"', '*').replace(",,", "*").replace('/', 'a') : "null").append(",");
        row.append("schoolName: ").append(!punkt.getSchoolName().equals("") ? punkt.getSchoolName().replace('"', '*').replace(',','+').replace(",,", "*").replace('/', 'a') : "null").append(",");
        row.append("zipcode: ").append(!punkt.getZipCode().equals("") ? punkt.getZipCode() : "null").append(",");
